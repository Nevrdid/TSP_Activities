...
Timer::Timer(const std::string& pid)
    : running(true), is_file_mode(false)
{
...
Timer::Timer(const std::string& file_path, bool file_mode)
    : running(true), is_file_mode(file_mode), file_to_watch(file_path)
{
    if (!file_mode) {
    ...
    watch_fd = inotify_add_watch(inotify_fd, watch_dir.c_str(), 
                ...
                for (char* ptr = buffer; ptr < buffer + len; ptr += sizeof(struct inotify_event) + event->len) {
                    ...
                    if (event->len > 0 && std::string(event->name) == watch_filename) {
                        if (event->mask & (IN_CREATE | IN_MOVED_TO)) {
                            if (!file_present) {
                                file_present = true;
                                std::cout << "File created/moved in: " << file_to_watch << std::endl;
                            }
                        } else if (event->mask & (IN_DELETE | IN_MOVED_FROM)) {
                            if (file_present) {
                                file_present = false;
                                std::cout << "File deleted/moved out: " << file_to_watch << std::endl;
